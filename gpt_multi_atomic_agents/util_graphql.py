...
def parse_out_mutation_names_from_schemas(accepted_graphql_schemas: list[str]) -> list[str]:
...
def filter_to_matching_mutation_calls(previously_generated_mutation_calls: list[str], accepted_mutation_names: list[str]) -> list[str]:
    accepted_mutation_names__adjusted = util_list.flatten([ [f"{n}(", f"{n} ("] for n in accepted_mutation_names])

    matching: list[str] = []
    for previous in previously_generated_mutation_calls:
        if any( list(filter(lambda a: a in previous, accepted_mutation_names__adjusted))):
