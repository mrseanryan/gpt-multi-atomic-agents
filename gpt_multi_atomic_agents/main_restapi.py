...
class GeneratePlanRequest(CustomBaseModel):
    agent_descriptions: list[prompts_router.AgentDescription] = Field(description="The descriptions of the available Agents. The response will contain the most suitable agents to execute in order.", examples=[ 
        [{
    ...
    chat_agent_description: str = Field(description="Describes the 'fallback' chat agent: if no suitable agents are recommended, this chat agent will be recommended, if the user's prompt is supported. The description should include the purpose and domain of this chat system.", examples=["Handles users questions about an ecosystem game like Sim Life"])
    user_prompt: str = Field(description="The input from the user", examples=["Add a goat instead of a sheep"])
    previous_plan: prompts_router.AgentExecutionPlanSchema | None = Field(description="Optionally also send a previously generated plan, so the AI can generate a new plan taking into account the user's feedback (in user_prompt).",
                                                                          examples=[
...
class FunctionAgentDefinitionMinimal(CustomBaseModel):
    agent_name: str = Field(description="The name of the agent", examples=[creature_agent_name])
    description: str = Field(
        description="The description of this agent, its purpose and capabilities."
    )
    accepted_functions: list[FunctionSpecSchema] = Field(description="The set of 'input' function calls that this agent understands. Each agent should understand its own output, but can also understand a subset of the output of other agents. This allows the agents to collaborate.")
    functions_allowed_to_generate: list[FunctionSpecSchema] = Field(description="The set of 'output' function calls that this agent generates.")
...
class FunctionCallGenerateRequest(CustomBaseModel):
    agent_definitions: list[FunctionAgentDefinitionMinimal] = Field(description="The defintions of the Agents to execute, in order.", examples=[
        [
        {
            "agent_name": creature_agent_name,
            "description": "Creates new creatures given the user prompt. Ensures that ALL creatures mentioned by the user are created, but ONLY if they are not already in graphql_data.",
    ...
    chat_agent_description: str = Field(description="Describe the purpose and domain of this chat system.", examples=["Handles users questions about an ecosystem game like Sim Life"])
    user_prompt: str = Field(description="The input from the user", examples=["Add a sheep that eats grass"])
    blackboard: FunctionCallBlackboard| None = Field(description="Optionally include the previous Blackboard state, to have a conversation (avoids stateless server). This contains previous state and new data (which the user has updated either by executing its implementation of Function Calls).", default=None)
    execution_plan: prompts_router.AgentExecutionPlanSchema | None = Field(description="Optionally also include a previously generated plan, to reduce latency. If no plan is included, then generate will also internally call generate_plan.",
                                                                           examples=[
{
  "chat_message": "Certainly! I'll help you add a sheep that eats grass to your ecosystem. I'll use the Creature Creator agent to create this new creature for you.",
  "recommended_agents": [
...
def _build_agent_definition_from_minimal(minimal_agent: FunctionAgentDefinitionMinimal) -> FunctionAgentDefinition:
...
def generate_function_calls(
    ...
    agent_definitions = [ _build_agent_definition_from_minimal(a) for a in request.agent_definitions]
