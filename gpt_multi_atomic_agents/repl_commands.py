...
commands: list[ReplCommandBase] = [DumpReplCommand(), HelpReplCommand(), QuitReplCommand()]
...
def check_user_prompt(user_prompt: str, blackboard: Blackboard) -> CommandAction:
    ...
    for command in commands:
        if _is_user_input_matching(user_prompt=user_prompt, aliases=command.get_aliases() + [command.get_name()]):
            return command.do(blackboard=blackboard)
    return CommandAction.no_action

def print_help() -> None:
    print_assistant_message_only(
    ...
    for command in commands:
        ...
        console.print(Text(f"  {command.get_name()} - {command.get_description()}{aliases}"))
...
# clear - clears out the blackboard, to start over
