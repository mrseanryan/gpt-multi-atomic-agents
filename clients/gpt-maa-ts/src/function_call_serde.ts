...
import { FunctionCallBlackboardAccessor, TypeScriptBlackboardFormat } from "./function_call_blackboard_accessor.js"
import { getConfig } from "./util_config.js"
import { printDetail, printError, readInputFromUser } from "./utils_print.js"
import { findFilesByExtension, readJsonFromFile, writeJsonToFile } from "./util_file.js";
...
export const load_blackboard_from_file = (filename: string): FunctionCallBlackboardAccessor | null => {
    const filepath = path.join(getConfig().localBlackboardsDirPath, filename)

    if (!fs.existsSync(filepath))
    {
        printError("That file does not exist. Please use the list command to view the current files.")
        return null
    }
    ...
    {
        console.error(e)        
        printError("Could not load that blackboard (the file could be old or a different format: FunctionCalling vs GraphQL)")
        return null
    }
}

export const save_blackboard_to_file = (blackboard: FunctionCallBlackboardAccessor, filename: string): void => {
...
export const list_blackboard_files = (blackboard: FunctionCallBlackboardAccessor|null): void => {
    const format = blackboard?.format ?? TypeScriptBlackboardFormat.function_call

    const files = findFilesByExtension(getConfig().localBlackboardsDirPath, `${format}.json`
    )
