...
export class FunctionRegistry
{
    ...
    public registerHandler(fun: FunctionSpecSchema, category: string, handler: IFunctionCallHandler): void {
        if (!fun.functionName)
            throw new Error("The function must have a name")

        if (this.functionNameToHandler[fun.functionName])
            throw new Error(`A handler is already registered for function ${fun.functionName}`);
    ...
    {
        if (!Object.keys(this.categoryToFunction).includes(category)) {
            printWarning(`No functions registered for category '${category}' - The default function handler will be used`)
            return []
        }
