import {
  ...
  FunctionAgentDefinitionMinimal_agent_parameters,
  FunctionSpecSchema,
 ...
 */
export function instanceOfSerializableAgentWithCategories(
  object: any
): object is SerializableAgentWithCategories {
  return (
    "agentName" in object &&
    "description" in object &&
    "acceptedFunctionCategories" in object &&
    "functionCategoriesAllowedToGenerate" in object &&
    "topics" in object &&
    "agentParameters" in object
  );
}
 ...
 */
export const convertSerializableAgentToContractAgent = (
  serializableAgent: SerializableAgentWithCategories,
  ...
  serializableAgent.agentParameters.forEach(
    (param) => (agentParametersDict[param] = [])
  );
  ...
  serializableAgent.acceptedFunctionCategories.forEach((category) => {
    acceptedFunctions = acceptedFunctions.concat(
      functionRegistry.getFunctionsForCategory(category)
    );
  });
  serializableAgent.functionCategoriesAllowedToGenerate.forEach((category) => {
    functionsAllowedToGenerate = functionsAllowedToGenerate.concat(
      functionRegistry.getFunctionsForCategory(category)
    );
  });
