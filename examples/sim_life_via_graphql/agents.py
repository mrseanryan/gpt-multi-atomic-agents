...
from gpt_multi_atomic_agents.graphql_dto import GraphQLAgentInputSchema, GraphQLAgentOutputSchema

def _read_schema(filename: str) -> str:
    return util_file.read_text_from_file(os.path.join(util_file.get_this_script_dir(__file__), "schemas", filename))
...
def build_relationship_agent():
    agent_definition = GraphQLAgentDefinition(
        ...
        accepted_graphql_schemas=[creatures_graphql, creature_mutations_graphql, vegetation_mutations_graphql, relationship_mutations_graphql],
        input_schema=GraphQLAgentInputSchema,
        initial_input=GraphQLAgentInputSchema(
